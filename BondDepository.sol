...
contract JumboBondDepository is Ownable {
  ...
  struct Bond {
    uint256 payout; // JUB remaining to be paid
  ...
  constructor(address _JUB, address _principle, address _treasury, address _DAO, address _bondCalculator) {
   ...
   *  @param _input uint
   */
  function setBondTerms(PARAMETER _parameter, uint256 _input) external onlyManager {
   ...
   *  @param _buffer uint
   */
  function setAdjustment(bool _addition, uint256 _increment, uint256 _target, uint256 _buffer) external onlyManager {
  ...
  function stakeOrSend(address _recipient, bool _stake, uint256 _amount) internal returns (uint256) {
    if (!_stake) {
      ...
      if (useHelper) {
        ...
        IERC20(JUB).approve(stakingHelper, _amount);
  ...
  function debtRatio() public view returns (uint256 debtRatio_) {
    uint256 supply = IERC20(JUB).totalSupply();
